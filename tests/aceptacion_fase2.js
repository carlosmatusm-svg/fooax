...
(async () => {
  ...
  const at = (c.atrasados || []).length;
  if (at) nota(at + " créditos atrasados 4 pagos o más (" + mx((c.atrasados || []).reduce((s, x) => s + x.saldo, 0)) + "). Lleva menos pagos de los que debería: Monse va a preguntar por ellos.");
  else ok("Ningún crédito atrasado 4 pagos o más", true);
